...
const peopleImc = people.map(x => ({
    ...x, 
    bmi: Math.round((x.weight / (x.height * x.height)) * 100) / 100,
}));

const peopleImcClass = peopleImc.map(x => ({
    ...x,
    classification: (x.bmi < 18.5 || x.bmi > 25) ? "Fora da faixa normal" : "Normal" 
}))


console.log(peopleImcClass);
